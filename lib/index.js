...
function cdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return cdf1( x, opts.alpha, opts.beta );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return cdf5( out, x, opts.alpha, opts.beta );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.alpha, opts.beta );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.alpha, opts.beta, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.alpha, opts.beta, opts.accessor );
		}
		return cdf2( out, x, opts.alpha, opts.beta );
	}
	return NaN;
} // end FUNCTION cdf()
