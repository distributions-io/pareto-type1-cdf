...
describe( 'matrix cdf', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			cdf( matrix( [10,10] ), mat, alpha, beta );
		}
	});

	it( 'should evaluate the Pareto cdf for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = cdf( actual, mat, alpha, beta );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( cdf( out, mat, alpha, beta ).data, expected );
